...
async def register_private_person(
    request: Request, data: ProjectCreate
) -> tp.Dict[str, tp.Any]:
    user_id = request.state.user_id
    project = BaseProjectCreateUpdate(
        owner=user_id, created_at=datetime.now(), **data.dict()
    )
...
@project_routes.get(
    "/get/{project_id}/", status_code=200, response_model=BaseProjectCreateUpdate
)
...
@project_routes.patch(
    "/patch/{project_id}/", status_code=200, response_model=BaseProjectCreateUpdate
)
async def update_user_me(
    request: Request, body: PatchProjectUpdateRequest, project_id: str
):
    user_id = request.state.user_id
    return await projects.update_project(
        project_id=project_id, instance=body, owner_id=user_id
    )


@project_routes.delete("/delete/{project_id}/", status_code=204)
async def delete_user_parser_by_id(
    request: Request,
    project_id: str,
):
    user_id = request.state.user_id
