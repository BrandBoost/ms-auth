...
async def update_project(
    project_id: str, instance: PatchProjectUpdateRequest, owner_id: str
):
    data = instance.dict(exclude_none=True)
    if not data:
        raise HTTPException(status_code=400, detail="Bad request.")

    await ProjectsRepository().update_by_id(
        instance_id=ObjectId(project_id), instance=data
    )
    return await ProjectsRepository().get_project_by_id(
