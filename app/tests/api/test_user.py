...
class TestUsers:
    ...
    async def test_update_user_me(self, async_client, private_user_with_token):
        response = await async_client.patch(
            ...
            json={"additional_info": {"some_info": "asdasdasd"}},
        )
        assert response.status_code == 200
    ...
    async def test_user_me_invalid_token(self, async_client, private_user_with_token):
        response = await async_client.get(
            url="/api/v1/users/me/", headers={"Authorization": "asdasdasdasdasd"}
        )
        assert response.status_code == 401

    @pytest.mark.asyncio
    async def test_register_private_person(
        self, async_client: AsyncClient, base_repository, global_dict
    ):
        person_payload = PrivatePersonCreateFactory.build()
        response = await async_client.post(
            "/api/v1/users/private_person/register/", json=person_payload.dict()
        )
    ...
    @pytest.mark.asyncio
    async def test_register_legal_person(
        self, async_client: AsyncClient, base_repository
    ):
        person_payload = LegalPersonCreateFactory.build()
        response = await async_client.post(
            "/api/v1/users/legal_person/register/", json=person_payload.dict()
        )
        payload = jwt.decode(
            response.json()["access_token"],
            settings.SECRET_KEY,
    ...
    async def test_unauthorized_login(self, async_client: AsyncClient, base_repository):
        ...
        response = await async_client.post(
            "/api/v1/users/login/", json=login_payload.dict()
        )
    ...
    async def test_forgot_password(
        ...
        user_data = {
            ...
            "additional_info": {"first_name": "first_name", "last_name": "last_name"},
        }
        ...
        response = await async_client.post(
            "/api/v1/users/forgot_password/", json=person_payload
        )
        assert response.status_code == 200
    ...
    async def test_refresh_token(
        ...
        response = await async_client.post(
            "/api/v1/users/refresh_token/", json=person_payload
        )
    ...
    async def test_activate(
        ...
        response = await async_client.post(
            f'/api/v1/users/activate/{global_dict["user_id"]}/'
        )
        assert response.status_code == 200
